...
func GetEngine() *haruka.Engine {
	...
	e.Router.POST("/library/{id:[0-9]+}/matchentity", newMatchEntityTask)
	e.Router.POST("/library/{id:[0-9]+}/syncindex", newSyncIndexTask)
	...
	e.Router.AddHandler("/link/{id:[0-9]+}/{type}/{token}/{any}", playLinkHandler)
	...
	e.Router.GET("/search", searchHandler)
